...
from data_loading import IdDataset as vid
...
from evaluation import evaluate_vid
try:
...
def test(model, queryloader, galleryloader, batch_size, use_gpu, ranks=[1, 5, 10], return_distmat=False):
    ...
    cmc, mAP = evaluate_vid(distmat, q_pids, g_pids, q_camids, g_camids, 10)
    ...
    #distmat_re = re_ranking(qf, gf, k1=15, k2=6, lambda_value=0.3)
    #print('Computing CMC and mAP')
    # cmc, mAP = evaluate(distmat, q_pids, g_pids, q_camids, g_camids, target_names)
    #cmc_re, mAP_re = evaluate_vid(distmat_re, q_pids, g_pids, q_camids, g_camids, 10)
...
def main():
    ...
    train_dir = '/home/jackie/datasets/vehicleid/image_train/'
    split_dir = '/home/jackie/datasets/vehicleid/train_split/'
    save_dir = './log/vehicleid/'
    pkl_path = './gms/vehicleid/'
    ...
    print("Running train_id.py")
    print('Currently using GPU ', gpu_id)
    cudnn.benchmark = True

    print('Initializing image data manager')
    dataset = init_imgreid_dataset(root=root_dir, name='vehicleID', test_size=test_size)
    num_train_pids = 0
    num_train_cams = 0
    train = []
    ...
    pid = 0
    print("Number of classes are ", class_names)
    pidx = {}
    for img_path, pid,_ in dataset.train:
        path = img_path[-17:]
        folder = path[0:5]
        pidx[folder] = pid
        pid+= 1
    ...
    data_tfr = vid(pkl_file= index_path, dataset = train, root_dir=train_dir, transform=transform_t)
    ...
    for epoch in range(start, max_epoch):
        ...
        for batch_idx, (img,label,index,pid, _) in enumerate(trainloader):
            ...
            for i in range(train_batch_size):
                ...
                while True:	
                    neg_label = random.choice(range(1, 27955))
                    n_l = strint(neg_label, 'vehicleid')
            ...
            #htri_loss = triplet_loss(features, margin, train_batch_size)
