...
def compute_gms_matches(orb: cv2.ORB, bf: cv2.BFMatcher, img1: np.ndarray, img2: np.ndarray):
...
def process_class(image_paths: list, image_size: tuple = (224, 224)):
    n = len(image_paths)
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):                   
            ...
            # Compute GMS matches
            matches = compute_gms_matches(orb, bf, img1, img2)
...
dict_index = {os.path.basename(image): (class_label, counter) for class_label, images in class_images.items() for counter, image in enumerate(images)}
...
with tqdm(total=total_iterations, desc="Processing pickle files") as pbar:
    for class_label, images in class_images.items():
        ...
        adj_matrix = process_class(images, image_size=image_size)
