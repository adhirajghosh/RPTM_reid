...
def build_transforms(height,
    ...
    transform_val = []
    transform_val += [Random2DTranslation(height, width)]
    transform_val += [RandomHorizontalFlip()]
    if color_jitter:
        transform_val += [ColorJitter(brightness=0.2, contrast=0.15, saturation=0, hue=0)]
    transform_val += [ToTensor()]
    if color_aug:
        transform_val += [ColorAugmentation()]
    transform_val += [normalize]
    if random_erase:
        transform_val += [RandomErasing()]
...
def test_transform(height, width):
    imagenet_mean = [0.485, 0.456, 0.406]
    imagenet_std = [0.229, 0.224, 0.225]
    normalize = Normalize(mean=imagenet_mean, std=imagenet_std)
