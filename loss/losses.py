...
def triplet_loss(features, margin, batch_size, size_average = True):
    ranking_loss = nn.MarginRankingLoss(margin=margin)
    #anchor = L2Normalization(features[0:batch_size])
    #positive = L2Normalization(features[batch_size:batch_size*2])
    #negative = L2Normalization(features[batch_size*2:batch_size*3])
    ...
    y = torch.ones_like(distance_negative)
    #losses = F.relu(distance_positive - distance_negative + margin)
    losses = ranking_loss(distance_positive, distance_negative, y)
