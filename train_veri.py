...
def main():
    ...
    train_dir = '/home/jackie/datasets/veri/image_train/'
    split_dir = '/home/jackie/datasets/veri/train_split/'
    save_dir = './log/veri/'
    pkl_path = './gms/veri/'
    ...
    print("Running train_veri.py for GMS-Min")
    print('Currently using GPU ', gpu_id)
    cudnn.benchmark = True

    print('Initializing image data manager')
    dataset = init_imgreid_dataset(root=root_dir, name='veri')
    num_train_pids = 0
    num_train_cams = 0
    train = []
    ...
    data_tfr = vd(pkl_file= index_path, dataset = train, root_dir=train_dir, transform=transform_t)
        ...
        for batch_idx, (img,label,index,pid, _) in enumerate(trainloader):
            ...
            for i in range(train_batch_size):
                ...
                while True:	
                    ...
                    if neg_label is not int(labelx) and os.path.isdir(os.path.join(split_dir, strint(neg_label, 'veri'))) is True:
                        break
            ...
            loss = lambda_htri*htri_loss+lambda_xent*xent_loss
